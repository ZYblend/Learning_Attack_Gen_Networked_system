...
for iter = 1:5
    subplot(5,1,iter)
    E12 = x(:,1+6*(iter-1):6*iter)-x(:,1+6*iter:6*(iter+1));
    E12d = (xd(:,iter)-xd(:,iter+1)).';
    dis_error = E12-E12d;
    plot(t(t<=50),dis_error(t<=50,:),'LineWidth',LW)
    dif = iter*10+iter+1;
    yname = "e_{"+num2str(dif)+"}";
    ylabel(yname)
    xlabel('Time')
    set(gca,'FontSize',FS)
end
sgtitle('Displacement error');
